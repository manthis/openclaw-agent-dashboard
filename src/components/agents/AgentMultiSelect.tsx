      ...
      <div className='flex flex-wrap gap-1 bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 min-h-[40px]'>
        {values.map((v) => (
          <span key={v} className='flex items-center gap-1 bg-slate-700 text-slate-200 text-xs px-2 py-0.5 rounded'>
            {getDisplay(v)}
            <button type='button' onClick={() => remove(v)} className='text-slate-400 hover:text-red-400 leading-none'>&times;</button>
          </span>
        ))}
        <input
          className='flex-1 min-w-[140px] bg-transparent text-slate-200 text-sm outline-none py-0.5'
          value={filter}
          ...
          {filtered.map((o) => (
            <li key={o.id}>
              ...
              >
                ...
                {o.id !== '*' && <span className='ml-1 text-slate-500 font-mono text-xs'>({o.id})</span>}
              </button>
            </li>
          ))}
        </ul>
