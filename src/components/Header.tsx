...
type SystemData = { gateway?: { connected?: boolean } };
...
export function Header() {
  ...
  const connected = Boolean(live?.gatewayConnected);

  const isDark = resolvedTheme === 'dark';

  const dotBase = 'relative inline-block w-4 h-4 rounded-full animate-pulse ring-2';
  ...
  const dotHaloClass = connected ? 'bg-emerald-400/55' : 'bg-rose-400/55';
      ...
      <div className="flex items-center gap-3 text-sm">
        <div className="flex items-center gap-2">
          <span className={`${dotBase} ${dotColor}`} style={dotGlowStyle}>
            ...
            <span className={`absolute -inset-2 rounded-full blur-md ${dotHaloClass}`} />
          </span>
          <span
